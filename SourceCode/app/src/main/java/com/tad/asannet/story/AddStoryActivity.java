...
public class AddStoryActivity extends AppCompatActivity {
    ...
    private void uploadImage_10(){
        ...
        if (mImageUri != null){
            ...
            uploadTask = fileReference.putFile(mImageUri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                @Override
                public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                    if(task.isSuccessful()){
                        Toast.makeText(AddStoryActivity.this, "Added", Toast.LENGTH_SHORT).show();
                        fileReference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                            @Override
                            public void onSuccess(Uri uri) {
                                miUrlOk = uri.toString();
...
////                                DatabaseReference reference = FirebaseDatabase.getInstance().getReference("Story")
////                                        .child(myid);
//                                final DocumentReference reference = mFirestore.collection("Story")
//                                        .document(myid);
//
//                                String storyid = reference.getId();
//                                long timeend = System.currentTimeMillis()+86400000; // 1 day later
//
//                                HashMap<String, Object> hashMap = new HashMap<>();
//                                hashMap.put("imageurl", miUrlOk);
...
//                                hashMap.put("userid", myid);
//
//                                reference.collection(storyid).add(hashMap);
//
//                                pd.dismiss();
//
//                                finish();
